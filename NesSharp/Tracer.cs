...
namespace NesSharp
{
    public class Tracer
    {
        ...
        public void Disassemble(byte op, int pc, byte a, byte x, byte y, byte ps, byte sp, int cycles)
        {
            ...
            {
                case "IMPL":
                    str1 = $"{pc:X4}  {op:X2}  {tab,5} {opnames[op][0]} {tab,28}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "ACCU":
                    str1 = $"{pc:X4}  {op:X2}  {tab,5} {opnames[op][0]} {tab,26}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "IMME":
                    b1 = c.mapper.CpuReadDebug(pc + 1);
                    str3 = $"{op:X2} {b1:X2}";
                    str1 = $"{pc:X4}  {str3}  {tab,2} {opnames[op][0]} #${b1:X2} {tab,23}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "ZERP":
                    b1 = c.mapper.CpuReadDebug(pc + 1);
                    ...
                    str3 = $"{op:X2} {b1:X2}";
                    str1 = $"{pc:X4}  {str3}  {tab,2} {opnames[op][0]} ${b1:X2} = {v:X2} {tab,19}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "ZERX":
                    ...
                    b3 = c.mapper.CpuReadDebug((byte)(b1 + 1));
                    v = c.mapper.CpuReadDebug(b1);
                    str3 = $"{op:X2} {b0:X2}";
                    str1 = $"{pc:X4}  {str3}  {tab,2} {opnames[op][0]} ${b0:X2},X @ {b1:X2} = {v:X2} {tab,12}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "ZERY":
                    b0 = c.mapper.CpuReadDebug(pc + 1);
                    ...
                    v = c.mapper.CpuReadDebug(b1);
                    str3 = $"{op:X2} {b0:X2}";
                    str1 = $"{pc:X4}  {str3}  {tab,2} {opnames[op][0]} ${b0:X2},Y @ {b1:X2} = {v:X2} {tab,12}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "ABSO":
                    b1 = c.mapper.CpuReadDebug(pc + 1);
                    ...
                    {
                        str1 = $"{pc:X4}  {str3}  {opnames[op][0]} ${addr:X4} {tab,22}";
                    }
                ...
                case "INDX":
                    ...
                    str3 = $"{op:X2} {b0:X2}";
                    str1 = $"{pc:X4}  {str3}  {tab,2} {opnames[op][0]} (${b0:X2},X) @ {(byte)(b0 + x):X2} = {addr:X4} = {v:X2} {tab,3}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "INDY":
                    b0 = c.mapper.CpuReadDebug(pc + 1);
                    ...
                    v = c.mapper.CpuReadDebug(addr);
                    str3 = $"{op:X2} {b0:X2}";
                    str1 = $"{pc:X4}  {str3}  {tab,2} {opnames[op][0]} (${b0:X2}),Y = {b1:X4} @ {addr:X4} = {v:X2} {tab,1}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "INDI":
                    b0 = c.mapper.CpuReadDebug(pc + 1);
                    ...
                    {
                        v = c.mapper.CpuReadDebug(addr) | (byte)c.mapper.CpuReadDebug((addr + 1)) << 8;
                    }

                    str3 = $"{op:X2} {b0:X2} {b1:X2}";
                    str1 = $"{pc:X4}  {str3}  {opnames[op][0]} (${addr:X4}) = {v:X4} {tab,13}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "RELA":
                    b1 = c.mapper.CpuReadDebug(pc + 1);
                    addr = pc + (sbyte)(b1) + 2;
                    str3 = $"{op:X2} {b1:X2}";
                    str1 = $"{pc:X4}  {str3}  {tab,2} {opnames[op][0]} ${addr:X4} {tab,22}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
                case "NULL":
                    str1 = $"{pc:X4}  {op:X2}  {tab,5} NOP {tab,28}";
                    str2 = $"A:{a:X2} X:{x:X2} Y:{y:X2} P:{ps:X2} SP:{sp:X2}";
                    break;
            }

            str2 += $" PPU:{c.cpu.ppucycles,3},{c.ppu.ppu_scanline,3} CYC:{cycles}";
            tracelines.Add(str1 + str2);
            Console.WriteLine(str1 + str2);
        }

        public void DisassembleFCEUXFormat(byte op, int pc, byte a, byte x, byte y, byte ps, byte sp, int cycles)
        {
            string strpc = "";
            string strins = "";
            int b0 = 0;
            ...
            switch (mode)
            {
                ...
                case "INDX":
                    ...
                    strins = $"{opnames[op][0]} (${b0:X2},X) @ ${(byte)(b0 + x):X2} = #${v:X2}";
                    //str1 += $"${pc:X4}:{op:X2} {b0,-11:X2} {opnames[op][0]} (${b0:X2},X) @ {(byte)(b0 + x):X2} = {addr:X4} = {v,-42:X2}";
                    break;
                case "INDY":
                    b0 = c.mapper.CpuReadDebug(pc + 1);
                    b1 = (byte)(b0);
